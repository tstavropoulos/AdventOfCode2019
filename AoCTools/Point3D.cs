...
namespace AoCTools
{
    public readonly struct Point3D
    {
        ...
        public readonly int Length => Math.Abs(x) + Math.Abs(y) + Math.Abs(z);

        public readonly Point2D ToPoint2D => new Point2D(x, y);
        public readonly LongPoint2D ToLongPoint2D => new LongPoint2D(x, y);
        public readonly LongPoint3D ToLongPoint3D => new LongPoint3D(x, y, z);
        public readonly Vector2D ToVector2D => new Vector2D(x, y);
        public readonly Vector3D ToVector3D => new Vector3D(x, y, z);

        public readonly Point3D AxisStep
        {
            get
            {
                if (x == 0 && y == 0 && z == 0)
        ...
        public static implicit operator (int x, int y, int z)(in Point3D point) =>
